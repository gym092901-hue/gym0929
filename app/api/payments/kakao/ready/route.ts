...
export async function POST(request: NextRequest) {
  if (isDemoModeEnabled()) {
    return NextResponse.json(
      { error: "DEMO_MODE에서는 실제 카카오페이 API를 호출하지 않습니다." },
      { status: 403 },
    );
  }

  let body: KakaoReadyBody;

  try {
    body = (await request.json()) as KakaoReadyBody;
  } catch {
    return NextResponse.json(
  ...
  try {
    ...
    return NextResponse.json({
      ...
      redirectUrl: payment.nextUrl,
    });
  } catch (error) {
    console.error("KakaoPay ready route failed", {
      readingId: body.readingId,
