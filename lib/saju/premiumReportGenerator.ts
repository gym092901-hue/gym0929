...
> = {
  wood: {
    ...
    love:
      "같이 걷고, 같이 보고, 보호자의 반응을 확인하면서 애정을 키워가는 편이에요.",
    routine:
  ...
  fire: {
    ...
    gift:
      "분위기를 환하게 만들고, 보호자에게 반응을 보여주며 교감의 온도를 높여주는 점이 큰 장점이에요.",
    sensitivity:
  ...
  earth: {
    ...
    routine:
      "식사, 산책 또는 놀이, 휴식의 순서가 일정할수록 마음이 편안해져요.",
    year:
  ...
  water: {
    ...
    sensitivity:
      "계속 반응을 요구받거나, 쉴 틈 없이 자극이 이어지거나, 조용히 살펴볼 시간이 짧으면 부담을 느낄 수 있어요.",
    love:
...
const catMonthlyChecklist = [
  ...
  "긴 놀이보다 짧은 집중 놀이가 잘 맞을 수 있어요. 장난감을 바꿀 때는 하나씩 꺼내 반응을 보고, 쉬는 시간을 충분히 남겨주세요.",
  "놀이 리듬을 다시 회복하기 좋은 달이에요. 같은 시간대에 짧은 사냥 놀이를 열어주고, 좋아하는 이동 경로를 관찰해보세요.",
  "새로운 자극을 천천히 더하기 좋아요. 낯선 장난감이나 박스는 냄새를 맡고 지나갈 시간을 주며 반응을 기록해보세요.",
...
function fixNameSpacing(report: string, petName: string) {
  ...
  return report
    .replace(new RegExp(`${escapedName}\\s+의`, "g"), `${petName.trim()}의`)
    .replace(new RegExp(`${escapedName}\\s+에게`, "g"), `${petName.trim()}에게`)
    .replace(new RegExp(`${escapedName}\\s+이(?=\\s|[,.!?]|$)`, "g"), postposition.subject(petName))
    .replace(new RegExp(`${escapedName}\\s+가(?=\\s|[,.!?]|$)`, "g"), postposition.subject(petName))
    .replace(new RegExp(`${escapedName}\\s+은(?=\\s|[,.!?]|$)`, "g"), postposition.topic(petName))
    .replace(new RegExp(`${escapedName}\\s+는(?=\\s|[,.!?]|$)`, "g"), postposition.topic(petName))
    .replace(new RegExp(`${escapedName}\\s+을(?=\\s|[,.!?]|$)`, "g"), postposition.object(petName))
    .replace(new RegExp(`${escapedName}\\s+를(?=\\s|[,.!?]|$)`, "g"), postposition.object(petName));
}
...
${basisLabel}을 기준으로 간단히 살핀 ${namePossessive} 오행 흐름은 숫자로 평가하기보다 행동 언어로 읽는 편이 더 다정합니다. ${elementBalanceText(primaryLabel, secondaryLabel)} 오행은 보호자가 아이를 평가하는 도구가 아니라, 아이가 어떤 방식으로 안정감을 느끼고 어떤 자극에 마음이 열리는지 살피는 언어에 가깝습니다. ${primaryText.core} ${secondaryText.core} 이 두 기운이 함께 있을 때 ${nameTopic} ${world.movement} 같은 일상의 단서에 민감하게 반응하면서도, 보호자의 분위기를 통해 다시 자기 자리를 찾아가려는 경향이 보여요. 특히 ${petKind}의 생활에서는 몸으로 표현되는 신호가 중요합니다. 가까이 오는 거리, 시선의 길이, 머무는 자리, 놀이 후 쉬는 방식 같은 작은 장면을 보면 ${namePossessive} 오행 기질이 훨씬 자연스럽게 보일 수 있어요.`,
    ...
    `3. 성격의 장점
${namePossessive} 가장 예쁜 장점은 자기만의 속도로 관계를 쌓아간다는 점이에요. ${primaryText.gift} ${secondaryLabel}의 기운까지 함께 보면 ${secondaryText.gift} 이런 모습도 자연스럽게 섞여요. 보호자가 보기에는 아주 작은 변화처럼 보여도, ${nameTo}는 큰 의미가 있는 행동일 수 있어요. 예를 들어 평소보다 조금 더 가까운 자리를 고르거나, 부르면 한 박자 늦게라도 돌아보거나, 놀이가 끝난 뒤 보호자 주변에 머무는 모습은 모두 ${nameSubject} 자기 방식으로 신뢰를 표현하는 장면일 수 있습니다. 너무 빠른 기대보다 작은 신호를 알아봐주는 보호자와 함께 있을 때 ${namePossessive} 장점은 더 또렷하게 살아나요. 이 아이는 보호자를 기쁘게 하려고 무리해서 맞추기보다, 편안하다고 느끼는 순간에 가장 자기다운 사랑스러움을 보여주는 경향이 있어요.`,

    `4. 예민해지기 쉬운 상황
${nameSubject} 예민해지는 순간은 대개 마음의 준비보다 자극이 먼저 들어올 때예요. ${primaryText.sensitivity} 여기에 ${secondaryLabel}의 결이 더해지면 ${secondaryText.sensitivity} 이런 상황에서도 잠깐 멈추거나, 고개를 돌리거나, 보호자의 반응을 먼저 보는 모습이 나타날 수 있어요. 이것을 고집이나 문제로만 보기보다 “지금 이 아이가 무엇을 확인하고 싶어 할까?”라고 바라봐주면 훨씬 다정한 해석이 됩니다. ${nameTopic} 불편함을 말로 설명하지 못하니까 몸의 방향, 숨는 위치, 꼬리나 귀의 작은 변화, 시선의 움직임으로 자기 마음을 알려줄 수 있어요. 보호자는 그 신호를 발견했을 때 바로 더 큰 자극을 주기보다, 한 걸음 물러나고 익숙한 목소리로 짧게 안내해주는 편이 좋아요. 무조건 피하게 하기보다 안전한 거리에서 살필 시간을 주면, ${nameTopic} 자기 속도로 다시 편안함을 찾을 수 있어요.`,
...
월별 조언은 운명을 정해놓는 달력이 아니라, 보호자가 한 달씩 생활을 돌아볼 수 있도록 만든 작은 체크리스트에 가까워요. ${nameSubject} 잘 반응했던 놀이, 조금 불편해했던 상황, 좋아했던 휴식 공간을 짧게 메모해보세요. 몇 달이 지나면 ${namePossessive} 패턴이 훨씬 선명하게 보일 거예요. 이 기록은 거창할 필요가 없습니다. “오늘은 창가에서 오래 쉬었다”, “낯선 소리에 잠깐 멈췄지만 금방 돌아왔다”, “짧은 놀이 뒤에 더 편안해 보였다” 같은 한 줄이면 충분해요. ${nameTopic} 보호자가 이렇게 자기 신호를 알아봐주는 것만으로도 더 안정된 일상을 경험할 수 있습니다.`,

    `10. 보호자에게 전하는 메시지
${nameObject} 가장 잘 이해하는 방법은 빠르게 결론을 내리는 것이 아니라, 반복되는 작은 장면을 오래 바라보는 것입니다. ${nameTopic} 이미 보호자에게 많은 이야기를 하고 있어요. 다만 그 이야기가 말이 아니라 눈빛, 몸의 방향, 다가오는 거리, 쉬는 자리, 놀이를 시작하고 끝내는 속도로 표현될 뿐입니다. 보호자가 그 신호를 알아봐주면 ${nameTopic} “내가 이해받고 있구나”라는 안정감을 조금씩 쌓아갈 수 있어요. 너무 완벽한 보호자가 되려고 애쓰지 않아도 괜찮습니다. 같은 목소리로 불러주고, 기다려주고, 성공한 순간에 작게 칭찬해주는 것만으로도 ${nameTo}는 충분히 따뜻한 기준이 됩니다. 이 리포트는 불안을 만들기 위한 글이 아니라, 보호자가 ${nameObject} 더 다정하게 바라볼 수 있도록 돕는 안내서입니다. 오늘도 ${nameTopic} 자기만의 방식으로 보호자를 믿고, 확인하고, 곁에 머물고 있을 가능성이 커요. 그 마음을 천천히 받아주세요.`,
  ];
