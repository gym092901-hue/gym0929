...
export function generatePremiumReport(input: PremiumReportInput) {
  ...
  const loveGuidance =
    input.type === "cat"
      ? `손을 먼저 뻗기보다 ${nameSubject} 먼저 다가올 때까지 기다리고, 느린 눈맞춤과 낮은 목소리로 반응해주면 보호자는 ${nameTo} 아주 믿을 만한 기준점이 됩니다.`
...
월별 조언은 운명을 정해놓는 달력이 아니라, 보호자가 한 달씩 생활을 돌아볼 수 있도록 만든 작은 체크리스트에 가까워요. ${nameSubject} 잘 반응했던 놀이, 조금 불편해했던 상황, 좋아했던 휴식 공간을 짧게 메모해보세요. 몇 달이 지나면 ${namePossessive} 패턴이 훨씬 선명하게 보일 거예요. 이 기록은 거창할 필요가 없습니다. ${monthlyRecordExample} ${nameTopic} 보호자가 이렇게 자기 신호를 알아봐주는 것만으로도 더 안정된 일상을 경험할 수 있습니다.`,
